...
- 浏览和播放歌曲、专辑、艺术家、播放列表与网络电台，电台支持 HTTP(S)、HLS、RTSP 和 MMS（MMSH/MMST）
...
缓存目录必须使用绝对路径，`concurrency` 用于限制同时提取并写入缓存的封面数量，可设置为 `1` 到 `64`。Docker 部署中的 `/data` 已持久化；缓存键由数据库中的歌曲 ID 和文件修改时间组成，修改曲库挂载路径不会使缓存失效，文件修改时间变化后会自动重新提取。缓存命中时只读取本地缓存，不访问曲库文件。设置为 `enabled: false` 时不会读写封面缓存。
